...
\begin{document}
    ...
    \begin{table}[H]
        ...
        \hline
            ...
            \hline
            \emph{\tsc{Plural}} & & & & \\
            \tsc{Nom} & \makecell{\grc{οἵδε} \trc{hoide} \\ \small these (here)} & \makecell{\grc{αἵδε} \trc{haide} \\ \small these (here)} & \makecell{\grc{τάδε} \trc{tade} \\ \small these (here)} & \makecell{( -- ) \\ \small that thing} \\
            \tsc{Gen} & \makecell{\grc{τῶνδε} \trc{tōnde} \\ \small of these (here)} & \makecell{\grc{τῶνδε} \trc{tōnde} \\ \small of these (here)} & \makecell{\grc{τῶνδε} \trc{tōnde} \\ \small of these (here)} & \makecell{( -- ) \\ \small that thing} \\
            \tsc{Dat} & \makecell{\grc{τοῖσδε} \trc{toisde} \\ \small to/for these (here)} & \makecell{\grc{ταῖσδε} \trc{taisde}\\ \small to/for these (here)} & \makecell{\grc{τοῖσδε} \trc{toisde} \\ \small to/for these (here)} & \makecell{( -- ) \\ \small that thing} \\
            \tsc{Acc} & \makecell{\grc{τούσδε} \trc{tousde} \\ \small these (here)} & \makecell{\grc{τάσδε} \trc{tasde} \\ \small these (here)} & \makecell{\grc{τάδε} \trc{tade} \\ \small these (here)} & \makecell{( -- ) \\ \small that thing} \\
        \end{tabular}
