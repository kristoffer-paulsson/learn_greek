...
\begin{document}
    ...
    A demonstrative pronoun is a deictic element whose reference is determined by the context of utterance.
    \textbf{Deixis} captures this dependence on the speaker’s \textit{origo} and is commonly divided into spatial,
    temporal, and discursive domains. Spatial deixis locates referents relative to the speaker; temporal
    deixis locates them relative to the time of utterance; discursive deixis locates them within the
    unfolding text.

    Proximal demonstratives encode proximity to the speaker’s deictic center. While not first-person forms,
    they are intrinsically speaker-oriented and thus aligned with first-person perspective.

    Ancient Greek has a three-way demonstrative system. \grc{ὅδε} is the strongly proximal term, marking referents
    as immediately accessible to the speaker, often with physical or rhetorical deixis and frequently introducing
    new discourse referents. It contrasts with \grc{οὗτος} (medial, discourse-accessible or addressee-oriented) and
    \grc{ἐκεῖνος} (distal, remote in space, time, or discourse).



    %What is a demonstrative pronoun.
    %What is deixis in a spatial, temporal or discursive context.
    %Specifically what is the proximal demonstrative and how does it relate to first person,
    %\grc{ὅδε} as a ancient greek proximal pronoun.

    Groton (2013:77) states that \grc{ὅδε} ``points out [i.] someone or something very close to the speaker \emph{or} points to [ii.] what will follow in the next sentence.''
