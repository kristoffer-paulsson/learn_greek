...
\begin{document}
    ...
    \subsection*{Koine Greek Proximal Demonstrative Pronoun \grc{ὅδε} for this/these}
    ...
    \begin{table}[H]
        \begin{tabular}{c|ccc}
            \textbf{Inflection} & \textbf{Masc} & \textbf{Fem} & \textbf{Neut.} \\
            \hline
            \emph{\tsc{Singular}} \\
            \tsc{Nom} & \makecell{\grc{ὅδε} \trc{hode} \\ \small this (masc.)} & \makecell{\grc{ἥδε} \trc{hēde} \\ \small this (fem.)} & \makecell{\grc{τόδε} \trc{tode} \\ \small this (neut.)} \\
            \tsc{Gen} & \makecell{\grc{τοῦδε} \trc{toude} \\ \small of this (masc.)} & \makecell{\grc{τῆσδε} \trc{tēsde} \\ \small of this (fem.)} & \makecell{\grc{τοῦδε} \trc{toude} \\ \small of this (neut.)} \\
            \tsc{Dat} & \makecell{\grc{τῷδε} \trc{tōide} \\ \small to/for this (masc.)} & \makecell{\grc{τῇδε} \trc{tēide} \\ \small to/for this (fem.)} & \makecell{\grc{τῷδε} \trc{tōide} \\ \small to/for this (neut.)} \\
            \tsc{Acc} & \makecell{\grc{τόνδε} \trc{tonde} \\ \small this (masc.)} & \makecell{\grc{τήνδε} \trc{tēnde} \\ \small this (fem.)} & \makecell{\grc{τόδε} \trc{tode} \\ \small this (neut.)} \\
            \hline
            \emph{\tsc{Dual}} \\
            \tsc{Nom/Acc} & \makecell{\grc{τώδε} \trc{tōde} \\ \small these two (masc.)} & \makecell{\grc{τώδε} \trc{tōde} \\ \small these two (fem.)} & \makecell{\grc{τώδε} \trc{tōde} \\ \small these two (neut.)} \\
            \tsc{Dat/Gen} & \makecell{\grc{τοῖνδε} \trc{toinde} \\ \small of/to these two (masc.)} & \makecell{\grc{τοῖνδε} \trc{toinde} \\ \small of/to these two (fem.)} & \makecell{\grc{τοῖνδε} \trc{toinde} \\ \small of/to these two (neut.)} \\
            \hline
            \emph{\tsc{Plural}} \\
            \tsc{Nom} & \makecell{\grc{οἵδε} \trc{hoide} \\ \small these (masc.)} & \makecell{\grc{αἵδε} \trc{haide} \\ \small these (fem.)} & \makecell{\grc{τάδε} \trc{tade} \\ \small these (neut.)} \\
            \tsc{Gen} & \makecell{\grc{τῶνδε} \trc{tōnde} \\ \small of these (masc.)} & \makecell{\grc{τῶνδε} \trc{tōnde} \\ \small of these (fem.)} & \makecell{\grc{τῶνδε} \trc{tōnde} \\ \small of these (neut.)} \\
            \tsc{Dat} & \makecell{\grc{τοῖσδε} \trc{toisde} \\ \small to/for these (masc.)} & \makecell{\grc{ταῖσδε} \trc{taisde}\\ \small to/for these (fem.)} & \makecell{\grc{τοῖσδε} \trc{toisde} \\ \small to/for these (neut.)} \\
            \tsc{Acc} & \makecell{\grc{τούσδε} \trc{tousde} \\ \small these (masc.)} & \makecell{\grc{τάσδε} \trc{tasde} \\ \small these (fem.)} & \makecell{\grc{τάδε} \trc{tade} \\ \small these (neut.)} \\
        \end{tabular}
        \caption{Compiled from standard Ancient Greek grammars, enhanced with transliteration and grammatical-gender glosses; proximal (near speaker). (Smyth, 1956:94, \S333; Rydberg-Cox, n.d.)}
    \end{table}
