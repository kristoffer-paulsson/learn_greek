...
\begin{document}
    ...
    first-person perspective. Ancient Greek has a three-way demonstrative system, in which ὅδε functions as the
    strongly proximal term. It refers to its referents as \textit{this} or \textit{these} in the sense
    ...
    to something which is about to be mentioned.'' Similarly, Groton (2013:77) agrees to that \grc{ὅδε}
    ...
    \begin{tabular}{c|ccc}
        ...
        \hline
        \emph{\tsc{Singular}} \\
        \tsc{Nom} & \makecell{\grc{ὅδε} \trc{hode} \\ \small this (man)} & \makecell{\grc{ἥδε} \trc{hēde} \\ \small this (woman)} & \makecell{\grc{τόδε} \trc{tode} \\ \small this (thing/one)} \\
        \tsc{Gen} & \makecell{\grc{τοῦδε} \trc{toude} \\ \small of this (man)} & \makecell{\grc{τῆσδε} \trc{tēsde} \\ \small of this (woman)} & \makecell{\grc{τοῦδε} \trc{toude} \\ \small of this (thing/one)} \\
        \tsc{Dat} & \makecell{\grc{τῷδε} \trc{tōide} \\ \small to this (man)} & \makecell{\grc{τῇδε} \trc{tēide} \\ \small to this (woman)} & \makecell{\grc{τῷδε} \trc{tōide} \\ \small to this (thing/one)} \\
        \tsc{Acc} & \makecell{\grc{τόνδε} \trc{tonde} \\ \small this (man)} & \makecell{\grc{τήνδε} \trc{tēnde} \\ \small this (woman)} & \makecell{\grc{τόδε} \trc{tode} \\ \small this (thing/one)} \\
        \hline
        \emph{\tsc{Dual}} \\
        \tsc{Nom/Acc} & \makecell{\grc{τώδε} \trc{tōde} \\ \small these two (men)} & \makecell{\grc{τώδε} \trc{tōde} \\ \small these two (women)} & \makecell{\grc{τώδε} \trc{tōde} \\ \small these two (things)} \\
        \tsc{Dat/Gen} & \makecell{\grc{τοῖνδε} \trc{toinde} \\ \small of/to these two (men)} & \makecell{\grc{τοῖνδε} \trc{toinde} \\ \small of/to these two (women)} & \makecell{\grc{τοῖνδε} \trc{toinde} \\ \small of/to these two (things)} \\
        \hline
        \emph{\tsc{Plural}} \\
        \tsc{Nom} & \makecell{\grc{οἵδε} \trc{hoide} \\ \small these (men)} & \makecell{\grc{αἵδε} \trc{haide} \\ \small these (women)} & \makecell{\grc{τάδε} \trc{tade} \\ \small these (things)} \\
        \tsc{Gen} & \makecell{\grc{τῶνδε} \trc{tōnde} \\ \small of these (men)} & \makecell{\grc{τῶνδε} \trc{tōnde} \\ \small of these (women)} & \makecell{\grc{τῶνδε} \trc{tōnde} \\ \small of these (things)} \\
        \tsc{Dat} & \makecell{\grc{τοῖσδε} \trc{toisde} \\ \small to these (men)} & \makecell{\grc{ταῖσδε} \trc{taisde}\\ \small to these (women)} & \makecell{\grc{τοῖσδε} \trc{toisde} \\ \small to these (things)} \\
        \tsc{Acc} & \makecell{\grc{τούσδε} \trc{tousde} \\ \small these (men)} & \makecell{\grc{τάσδε} \trc{tasde} \\ \small these (women)} & \makecell{\grc{τάδε} \trc{tade} \\ \small these (things)} \\
    \end{tabular}
