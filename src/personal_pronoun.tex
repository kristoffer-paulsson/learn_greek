...
% Document
\begin{document}
    \subsection*{Ancient Greek: Personal Pronouns}
    ...
    First-person pronouns indicate the speaker, while second-person pronouns indicate the addressee. The third-person
    forms refer to entities outside the conversation, linking to an antecedent. They decline
    ...
    ``the forms \grc{μοῦ}, \grc{μοί}, \grc{μέ}; \grc{σοῦ}, \grc{σοί}, \grc{σέ}\dots~are enclitic.
    But if the pronoun is emphatic, the enclitic forms of the pronoun retain their accent, and in the first person
    the longer forms \grc{ἐμοῦ}, \grc{ἐμοί}, \grc{ἐμέ}, are then used. This generally happens also after prepositions.''
    ...
    % and ``yourselves''\linkfoot{yourselves}{https://dictionary.cambridge.org/dictionary/english/yourselves}{2025-05-13}
     ...
     \begin{table}[H]
         \begin{tabular}{c|ccc}
             & \tsc{1st} & \tsc{2nd} & \tsc{3rd} \\
             \hline
             \emph{\tsc{Singular}} \\
             \tsc{Nom} & \makecell{\grc{ἐγώ} \trc{egō} \\ \small I} & \makecell{\grc{σύ} \trc{sy} \\ \small you} & -- \\
             \tsc{Gen} & \makecell{\grc{ἐμοῦ} \trc{emou}, \grc{μου} \trc{mou} \\ \small my, mine} & \makecell{\grc{σοῦ} \trc{sou}, \grc{σου} \trc{sou} \\ \small your, yours}  & \makecell{\grc{οὗ} \trc{hou}, \grc{οὑ} \trc{hou} \\ \small his/hers/its} \\
             \tsc{Dat} & \makecell{\grc{ἐμοί} \trc{emoi}, \grc{μοι} \trc{moi} \\ \small to/for me} & \makecell{\grc{σοί} \trc{soi}, \grc{σοι} \trc{soi} \\ \small to/for you}  & \makecell{\grc{οἷ} \trc{hoi}, \grc{οἱ} \trc{hoi} \\ \small to/for him/her/it} \\
             \tsc{Acc} & \makecell{\grc{ἐμέ} \trc{eme}, \grc{με} \trc{me} \\ \small me} & \makecell{\grc{σέ} \trc{se}, \grc{σε} \trc{se} \\ \small you}  & \makecell{\grc{ἕ} \trc{he}, \grc{ἑ} \trc{he} \\ \small him/her/it} \\
             \hline
             \emph{\tsc{Dual}} \\
             ...
             \hline
             \emph{\tsc{Plural}} \\
             \tsc{Nom} & \makecell{\grc{ἡμεῖς} \trc{hēmeis} \\ \small we} & \makecell{\grc{ὑμεῖς} \trc{humeis} \\ \small yous}  & \makecell{\grc{σφεῖς} \trc{spheis} \\ \small they} \\
             \tsc{Gen} & \makecell{\grc{ἡμῶν} \trc{hēmōn} \\ \small our, ours} & \makecell{\grc{ὑμῶν} \trc{humōn} \\ \small your, yours}  & \makecell{\grc{σφῶν} \trc{sphōn} \\ \small their, theirs} \\
             \tsc{Dat} & \makecell{\grc{ἡμῖν} \trc{hēmin} \\ \small to/for us} & \makecell{\grc{ὑμῖν} \trc{humin} \\ \small to/for yous}  & \makecell{\grc{σφίσι(ν)} \trc{sphisi(n)} \\ \small to/for them} \\
             \tsc{Acc} & \makecell{\grc{ἡμᾶς} \trc{hēmas} \\ \small us} & \makecell{\grc{ὑμᾶς} \trc{humas} \\ \small yous}  & \makecell{\grc{σφᾶς} \trc{sphas} \\ \small them} \\
         \end{tabular}
         \caption{Compiled from standard Ancient Greek grammars enhanced with transliteration and cues. (Smyth, 1956:90, \S325)}
     \end{table}
